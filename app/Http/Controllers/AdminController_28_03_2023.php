...
class AdminController extends BaseController
{
    ...
    public function GetGeom(Request $request)
    {
        ...
        switch ($level) {
            ...
            case "forMauza":
                ...
                $sql = "SELECT DISTINCT plot_no_en as gid, plot_no_bn as label,m_code,plot_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' ORDER BY plot_no_en";
                $plots=DB::select($sql);
            ...
            case "forSheet":
                ...
                if($sheetVal == null) {
                    $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,plot_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' ORDER BY plot_no_en";
                    $result=DB::select($sql);
                }
                else {
                    $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,plot_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' AND sht_no_en = '$sheetVal' ORDER BY plot_no_en";
                    $result=DB::select($sql);
                }
    ...
    public function GetPlotInfo(Request $request){
        ...
        $projParam=DB::select($projParamsSql)[0]->proj4text;
        $sql = "SELECT * FROM $tableName WHERE ST_WITHIN(ST_SetSRID(ST_MakePoint($lon, $lat),4326),st_transform(geom,'$projParam',4326)) ORDER BY plot_no_en";
        $result=DB::select($sql);  
        //dd($sql);
        return response()->json($result);
    }
