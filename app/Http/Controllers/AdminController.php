...
class AdminController extends BaseController
{
    public function GetInfo(Request $request)
    {
        ...
        switch ($level) {
            case "forDiv":
                ...
                $sql = "SELECT code, name_en, concat(name_bn,'-',code::text) as name_bn FROM iwm_division WHERE is_uploaded=1 ORDER BY name_en";
            ...
            case "forDist":
                ...
                $sql = "SELECT code, name_en,concat(name_bn,'-',substring(code::text,3,5)) as name_bn, division_code FROM iwm_district WHERE is_uploaded=1 AND division_code = '$div'  ORDER BY name_en";
            ...
            case "forThana":
                ...
                $sql = "SELECT code, name_en,concat(name_bn,'-',substring(code::text,5,7)) as name_bn, division_code, district_code FROM iwm_upazila WHERE is_uploaded=1 AND district_code = '$dist' ORDER BY name_en";
            ...
            case "forMouzaByUpz":
                ...
                // ORDER BY m_name_en";
                //$sql = "SELECT DISTINCT geocode as code,concat(m_name_bn,'-',\"substring\"(geocode::text, 10, 12)) as name_bn, \"substring\"(m_code::text, 0,8) as m_code FROM gis_boundary_mauza_dynamic WHERE \"substring\"(geocode::text, 0, 7) = '$thana'";
                $sql = "SELECT code,name_bn,m_code FROM (
                    SELECT ROW_NUMBER() OVER(PARTITION BY geocode) as RON, geocode as code,concat(m_name_bn,'-',\"substring\"(geocode::text, 10, 12)) as name_bn, \"substring\"(m_code::text, 0,8) as m_code FROM gis_boundary_mauza_dynamic WHERE \"substring\"(geocode::text, 0, 7) = '$thana'
                ) A WHERE A.RON = 1";
                $result=DB::select($sql);
                break;
            case "forMauza":
    ...
    public function GetGeom(Request $request)
    {
        ...
        switch ($level) {
            ...
            case "forMauza":
                ...
                $plotOrLanduse = $request->plotOrLanduse;
                $sheets = [];
                $plots =[];
                $landuse = [];

                $projParamsSql = "SELECT proj4text FROM spatial_ref_sys WHERE srid = (SELECT FIND_SRID('public', '".$tableName."_mg', 'geom'))";
                $projParam=DB::select($projParamsSql)[0]->proj4text;

                if($plotOrLanduse =='landuse'){
                    $sql = "SELECT DISTINCT plot_no_en,m_code,plot_no_en,maj_class,sub_class,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_ml WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' ORDER BY plot_no_en";
                    $landuse=DB::select($sql);
                }
                else{
                    $sql = "SELECT DISTINCT sht_no_bn as sht_label, sht_no_en FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal'";
                    $sheets=DB::select($sql);
    
                    $sql = "SELECT DISTINCT plot_no_en as gid, plot_no_bn as label,m_code,plot_no_en,jl_no_en,sht_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' ORDER BY plot_no_en";
                    $plots=DB::select($sql);
                }           
            ...
            case "forSheet":
                ...
                if($plotOrLanduse =='landuse'){
                    $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,maj_class,sub_class,plot_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_ml WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' AND sht_no_en = '$sheetVal' ORDER BY plot_no_en";
                    $resultLandUse=DB::select($sql);
                }
                else{
                    if($sheetVal == null) {
                        $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,plot_no_en,jl_no_en,sht_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' ORDER BY plot_no_en";
                        $result=DB::select($sql);
                    }
                    else {
                        $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,plot_no_en,jl_no_en,sht_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' AND sht_no_en = '$sheetVal' ORDER BY plot_no_en";
                        $result=DB::select($sql);
                    }
                }
            ...
            case "forPlot":
                ...
                $resultLandUse = [];
                $plots = [];
                $sql = "SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,sht_no_en,plot_no_en,st_astext(st_transform(geom,'$projParam',4326)) as geom FROM ".$tableName."_mg WHERE \"substring\"(geocode_en::text, 0, 13) = '$mauzaVal' AND plot_no_en = '$plotVal' ORDER BY plot_no_en";
                $plot=DB::select($sql);
                //No need for now because mouza or sheet already loaded
    ...
    public function GetMauzaSummary(Request $request){
        ...
        $sql = "SELECT COUNT(gid) as total_plots FROM $mauza_tab_mg where \"substring\"(geocode_en, 0,13) = '$level' ";
        $result=DB::select($sql);
        ...
        (
            ...
            FROM $mauza_tab_ml 
            WHERE \"substring\"(geocode_en, 0,13) = '$level' AND maj_class_bn IS NOT NULL
        ...
        (
            SELECT 
                ...
                COUNT(plot_no_en) as plots 
            FROM $mauza_tab_ml 
            WHERE \"substring\"(geocode_en, 0,13) = '$level' AND sub_class_bn IS NOT NULL
    ...
    public function GetMauzaFilterdSummary(Request $request){
        $level = $request->mauzaVal; 
        $mauza_tab = $request->mauza_tab;
        $mauza_tab_ml = $mauza_tab.'_ml';
        $landuse = $request->landuse;
        $majorClassSql = "SELECT plots FROM
        (
            SELECT 
                COUNT(plot_no_en) as plots 
            FROM $mauza_tab_ml 
            WHERE \"substring\"(geocode_en, 0,13) = '$level' AND LOWER(maj_class) LIKE '%$landuse%' AND maj_class_bn IS NOT NULL
        ) A";
        $marjorClass = DB::select($majorClassSql);
        ...
        (
            SELECT 
                ...
                COUNT(plot_no_en) as plots 
            FROM $mauza_tab_ml 
            WHERE \"substring\"(geocode_en, 0,13) = '$level' AND LOWER(maj_class) LIKE '%$landuse%' AND sub_class_bn IS NOT NULL
            GROUP BY sub_class_bn
        ) A";
        $subClass = DB::select($subClassSql);

        return response()->json(array('major_class'=>$marjorClass[0],'sub_class'=>$subClass));
    }
    ...
    public function GetPlotInfo(Request $request){
        ...
        $projParamsSql = "SELECT proj4text FROM spatial_ref_sys WHERE srid = (SELECT FIND_SRID('public', '$tableName', 'geom'))";
        $projParam=DB::select($projParamsSql)[0]->proj4text;
        $sql = "SELECT  * FROM $tableName WHERE jl_no_en ='$jl' AND sht_no_en = '$sht' AND plot_no_en = '$plot' AND ST_WITHIN(ST_SetSRID(ST_MakePoint($lon, $lat),4326),st_transform(geom,'$projParam',4326)) ORDER BY plot_no_en";
        //dd($sql);
        $result=DB::select($sql);
        return response()->json($result);
    }
