    ...
    public function GetGeom(Request $request)
    {
        ...
        switch ($level) {
            ...
            case "forMauza":
                ...
                $sql = "SELECT st_astext(geom) as geom, m_name_bn as label FROM vw_gis_boundary_mouza WHERE geocode = '$mauzaVal' ";
                $result=DB::select($sql);
                ...
                $sql = 'SELECT DISTINCT sht_no_bn as sht_label, sht_no_en FROM view_mg WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\'';
                //dd($sql);
                $sheets=DB::select($sql);

                $sql = 'SELECT DISTINCT plot_no_en as gid, plot_no_bn as label,plot_no_en FROM view_mg WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\' ORDER BY plot_no_en';
                $plots=DB::select($sql);

                return response()->json(['result'=>$result,'sheets'=>$sheets,'plots'=>$plots]);
                break;
            ...
            case "forSheet":
                ...
                $sql = 'SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,plot_no_en,st_astext(geom) as geom FROM view_mg WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\' AND sht_no_en = \''.$sheetVal.'\' ORDER BY plot_no_en';
                $result=DB::select($sql);

                $sql = 'SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,maj_class,sub_class,plot_no_en,st_astext(geom) as geom FROM view_ml WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\' AND sht_no_en = \''.$sheetVal.'\' ORDER BY plot_no_en';
                $resultLandUse=DB::select($sql);
                return response()->json(['plots'=>$result,'land_use'=>$resultLandUse]);
                break;
            ...
            case "forPlot":
                ...
                $sql = 'SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,sht_no_en,plot_no_en,st_astext(geom) as geom FROM view_mg WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\' AND plot_no_en = \''.$plotVal.'\' ORDER BY plot_no_en';
                $plot=DB::select($sql);
                ...
                $sql = 'SELECT DISTINCT plot_no_en as gid,m_code, plot_no_bn as label,maj_class,sub_class,plot_no_en,st_astext(geom) as geom FROM view_ml WHERE "substring"(geocode_en::text, 0, 10) = \''.$mauzaVal.'\' AND sht_no_en = \''.$sheetVal.'\' ORDER BY plot_no_en';
                $resultLandUse=DB::select($sql);
                return response()->json(['plot'=>$plot,'plots'=>$plots,'land_use'=>$resultLandUse]);
                break;
        }
