...
.page-link {
    ...
    <div class="bg-body-light border-b">
        ...
        <nav class="breadcrumb bg-body-light mb-0" id="geoLocationBC">
                ...
                <a class="breadcrumb-item" href="{{ url('/district-dashboard') }}">District Dashboard</a>
            </nav>
        </div>
    </div>
    <!-- END Breadcrumb -->
    ...
    <div class="content" style="padding: 5px !important;">
        ...
        <div class="row js-appear-enabled animated fadeIn" data-toggle="appear">
                ...
                <div class="block">
                    <div class="block-content block-content-full" style="padding-bottom: 0px">
                        <div class=" text-center">
                            <div id="sub_class_pie" class="chart_div"></div>
                        </div>
                    </div>
                </div>
            </div> --}}
            ...
            <div class="col-md-12">
                <div class="Container">
                    <h3 style="margin-bottom: 0px" class="Head">জেলা অনুসারে আপলোডেড মৌজা ও সিট সংখ্যাঃ<span class="Arrows"></span></h3>
                    <!-- Carousel Container -->
                    <div class="SlickCarousel">
                      ...
                      @foreach ($distWiseCounts as $item)
                      <div class="ProductBlock">
                        <a class="block block-rounded block-bordered block-link-shadow" href="javascript:void(0)">
                            <div class="block-content block-content-full clearfix">
                                <div class="float-right mt-15 d-none d-sm-block">
                                ...
                                <div class="font-size-h3 font-w600 text-earth">{{$item->m_dist_bn}}</div>
                      ...
                      <div class="ProductBlock">
                        <a class="block block-rounded block-bordered block-link-shadow" href="javascript:void(0)">
                            <div class="block-content block-content-full clearfix">
                                ...
                                <div class="font-size-h3 font-w600 text-earth"><span data-toggle="countTo" data-speed="1000" data-to="" class="js-count-to-enabled">মোট জেলা</span></div>
                                <div class="font-size-sm font-w600 text-uppercase text-muted"><span class="count-osk">{{$counts[0]->dist}}</span></div>
                            </div>
                        </a>
                      </div>
                      <!-- Item -->
        ...
        <script>
            $(document).ready(function(){
                ...
                districts = districts.sort((a, b) => {
                    if (a.m_dist_bn < b.m_dist_bn) {
                        return -1;
                    }
                });
                console.log(districts);
                districts.forEach(item =>{
                    $district.push(item.m_dist_bn);
                });
                ...
                data.forEach(item =>{
                    ...
                    $data = [];
                    var distDetails = result.filter(x=> x.maj_class === item.maj_class && x.majclass_b === item.major_class);
                    //console.log('divDet',divDetails);
                    $district.forEach(dist => {
                        var item = distDetails.filter(x=>x.m_dist_bn == dist)[0];
                        console.log(item);
                        $data.push(parseFloat(item.hactor));
                    });
                    // distDetails.forEach(item=>{
                    //     $data.push({'dist':item.m_dist_bn,'hac':parseFloat(item.hactor)});
                    // });                    
                    $visible = false;
                ...
                //     //console.log(divisions[i].div_code);
                //     $data = [];
                //     var divDetails = result.filter(x=> x.div_code == divisions[i].div_code);
                //     divDetails.forEach(item=>{
                //         $data.push(parseFloat(item.hactor));
                //     });
                //     $majorclassColumnChartData.push({'name':divisions[i].m_div_bn,'data':$data});
                // }
                ...
                //   console.log('dist',$district);
                //   console.log('dist',$majorclassColumnChartData);
                ...
                //   Highcharts.chart('sub_class_pie', {
                //     chart: {
                //       plotBackgroundColor: null,
                //       plotBorderWidth: null,
                //       plotShadow: false,
                ...
                //     },
                //     title: {
                //       text: 'উপ শ্রেণী অনুসারে জমির পরিমাণঃ'
                //     },
                //     tooltip: {
                //       pointFormat: '{series.name}: <b>{point.hactor:.2f} ha, {point.percentage:.1f}%</b>'
                //     },
                //     accessibility: {
                //       point: {
                //         valueSuffix: 'ha'
