        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="js-wizard-simple block">
                    <!-- Wizard Progress Bar -->
                    <div class="progress rounded-0 wizard-height" data-wizard="progress">
                        <div class="progress-bar progress-bar-striped progress-bar-animated progress-bar-width" role="progressbar" aria-valuenow="30" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                    ...
                    <ul class="nav nav-tabs nav-tabs-alt nav-fill" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="#wizard-progress2-step3" data-toggle="tab">2. Role and Others</a>
                        </li>
                    </ul>
                    <!-- END Step Tabs -->
                    ...
                    <form action="{{ route('admin.users.update',$user->id) }}" method="post" enctype="multipart/form-data" role="form">
                        ...
                        @method('PUT')
                    <!-- Steps Content -->
                        <div class="block-content block-content-full tab-content tab-height">
                            <!-- Step 1 -->
                            <div class="tab-pane active" id="wizard-progress2-step1" role="tabpanel">
                                ...
                                <div class="form-group">
                                    <div class="form-material floating">
                                        <input class="form-control no-space" type="text" id="user_name" name="user_name" value="{{ $user->user_name }}" required="required">
                                        <label for="user_name">User ID</label>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="form-material floating">
                                        <input class="form-control no-space" type="email" id="email" name="email" value="{{ $user->email }}" required="required">
                                    ...
                                    <div class="form-material floating">
                                        <select name="role_id[]" id="role_id" size="1" class="form-control select2 role-select" multiple="multiple" required>
                                            @foreach ($assigned_roles as $assigned_role)
                                                <option value="{{ $assigned_role['id'] }}" selected>{{ $assigned_role['name'] }}</option>
                                            @endforeach
                                            @foreach ($unassigned_roles as $unassigned_role)
                                                <option value="{{ $unassigned_role['id'] }}">{{ $unassigned_role['name'] }}</option>
                                            @endforeach
                                        </select>
                                        {{--{{ Form::select('role_id', $roles + array(''=>'Select Role'), $user->role_id, ['class'=>'form-control', 'required', 'disabled'=>false, 'id'=>'role_id', 'size'=>'1']) }}--}}
                                ...
                                <div class="form-group">
                                    <div class="form-material floating">
                                        <select class="form-control" id="status" name="status" size="1">
                                            <option value="0" @if($user->is_active == 0) {{'selected'}} @endif>No</option>
                                            <option value="1" @if($user->is_active == 1) {{'selected'}} @endif>Yes</option>
                                        </select>
